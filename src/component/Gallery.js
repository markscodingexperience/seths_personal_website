...
const Gallery = () => {
    ...
    return (
        <section className="mx-auto relative z-10 p-4 px-1 md:px-30 lg:px-40 scroll-smooth ">
            <div className="columns-1 gap-4 md:columns-2 gap-5 lg:columns-3 gap-7 space-y-7 xl:columns-3 [&>img:not(:first-child)]:mt-5 ">
                {images.map((image,index) =>
                    <motion.div 
                        ...
                        variants={{
                            ...
                            visible:{opacity: 1, y: 0, duration: 2.5, delay: index * 3, transition:{
                                type: "spring",
                                bounce: 0.25
                              }}
                        }}
                        ...
                        viewport={{ once: false }}
                        >
                        <img src={image.image}  alt={image.name} className="object-cover w-full h-90  cursor-pointer" loading="lazy" onClick={() => handleClick(image.image)}/>
                    </motion.div>
            ...
            {selectedImage && (
                ...
                >
                    <img src={selectedImage} alt="Selected" className="w-full max-w-xs md:max-w-md lg:max-w-2xl xl:max-w-4xl max-h-screen object-contain" />
                </motion.div>
                {/* Close button */}
                <button
